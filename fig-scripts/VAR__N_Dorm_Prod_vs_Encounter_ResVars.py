...
def r_p_bootstrap(x, y, dat):
    ...
    for i in range(1):
        ...
        dat = dat.sample(n=500, replace = True)
        x1 = dat[x]
        y1 = dat[y]
...
#plt.ylim(0.5, 3.5)
plt.text(-1.0, 3.1, 'r = '+str(round(r,2)), fontsize=fs+1, color='k')
...
ax.get_yaxis().tick_left()
#plt.ylim(-1.0, 2.0)
plt.text(-1.0, 1.6, 'r = '+str(round(r,2)), fontsize=fs+1, color='k')
...
plt.savefig(mydir + '/results/figures/Var_Res_3by3.png', dpi=600, bbox_inches = "tight")
#plt.show()
#plt.close()
